...
async function saveAddress(addressData) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            addressData.id = data.id; // Get the DB assigned ID
        } else {
            addressData.id = Date.now().toString(); // Fallback ID
        }
    } catch (error) {
        console.error('Failed to save to DB, using local storage only', error);
        addressData.id = Date.now().toString(); // Fallback ID
    }
